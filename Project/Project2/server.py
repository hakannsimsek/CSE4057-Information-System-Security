...
def init_connection(client_socket):
    try:
        ...
        rsa_public_header = client_socket.recv(HEADER_LENGTH)

        username_length = int(username_header.decode("utf-8").strip())
        password_length = int(password_header.decode("utf-8").strip())
        rsa_public_length = int(rsa_public_header.decode("utf-8").strip())

        username = client_socket.recv(username_length)
        password = client_socket.recv(password_length)
        rsa_public = client_socket.recv(rsa_public_length)
        return {"username": username.decode("utf-8"), "password": password.decode("utf-8"),"rsa_public":rsa_public}
    except:
        return False
...
def receive_message(client_socket):
    try:
        ...
        if not len(message):
            return False
        if message_type.decode("utf-8") == "PMRES" and message.decode("utf-8") == "ACCEPT":
            cipher_info_header = client_socket.recv(HEADER_LENGTH)
            cipher_info = client_socket.recv(int(cipher_info_header.decode("utf-8").strip()))
            return {"username": username, "message_type": message_type, "message": message,
                "headers": [username_header, message_type_header, message_header],"cipher_info":cipher_info}
        elif message_type.decode("utf-8") == "PM":
            mac_header = client_socket.recv(HEADER_LENGTH)
            mac_field = client_socket.recv(int(mac_header.decode("utf-8").strip()))
            return {"username": username, "message_type": message_type, "message": message,
                    "headers": [username_header, message_type_header, message_header], "mac_field": mac_field}
...
def handle_login(server_name, server_header, server_socket):
    ...
    # user successfully logged in or registered
        ...
        user_rsa_public = user["rsa_public"]
        users_rsa_public_keys[user["username"]] = user_rsa_public

        server_rsa_public_header = f"{len(SERVER_RSA_PUBLIC.exportKey()) :< {HEADER_LENGTH}}".encode("utf-8")
        client_socket.send(
            server_header + message_type_header + msg_header + server_name + message_type + msg_.encode(
                "utf-8") + server_rsa_public_header + SERVER_RSA_PUBLIC.exportKey() )
        ...
        logger.info(f"Accepted new connection from {client_address[0]}:{client_address[1]} username:{user['username']} rsa_key:{user_rsa_public}")
...
def handle_pm_request(message, notified_socket, server_header, server_name):
    ...
    if pm_to_socket:
        ...
        user_signed_rsa_public_header = f"{len(users_rsa_public_keys[message['username'].decode('utf-8')]) :< {HEADER_LENGTH}}".encode("utf-8")

        pm_to_socket.send(
            server_header + message_type_header + message_header + server_name + "PMREQ".encode(
                "utf-8") + message_ + user_signed_rsa_public_header + users_rsa_public_keys[message['username'].decode('utf-8')] )
        logger.info(
            f"User: {message['username'].decode('utf-8')} requested private chat with user: {pm_to.split(' ')[1]}")
        print(f"User: {message['username'].decode('utf-8')} requested private chat with user: {pm_to.split(' ')[1]}")
        return
...
def handle_pm_response(message, notified_socket, server_header, server_name):
    ...
    if message["message"].decode("utf-8") == "ACCEPT":  # ACCEPT
        ...
        aes_iv_enc = message["cipher_info"]
        aes_iv_enc_header =  f"{len(aes_iv_enc) :< {HEADER_LENGTH}}".encode("utf-8")
        ...
        # create sign of server on users public key

        user_signed_rsa_public_header = f"{len(users_rsa_public_keys[message['username'].decode('utf-8')]) :< {HEADER_LENGTH}}".encode("utf-8")

        pm_partners[notified_socket].send(
            server_header + message['headers'][1] + msg_header + server_name + "PMRES".encode(
                "utf-8") + message_body + user_signed_rsa_public_header + users_rsa_public_keys[message['username'].decode('utf-8')] + aes_iv_enc_header + aes_iv_enc)
        logger.info(
            f"The PM start between {clients[requested_user_socket]['username']} and {clients[notified_socket]['username']}")
        print(f"The PM start between {clients[requested_user_socket]['username']} and {clients[notified_socket]['username']}")
        return
        ...
        for notified_socket in read_sockets:
            ...
            if notified_socket == server_socket:
                ...
                # user sends private chat
                    ...
                    mac_field_header =  f"{len(message['mac_field']) :< {HEADER_LENGTH}}".encode("utf-8")
                    partner_socket.send(
                        message['headers'][0] + message['headers'][1] + message['headers'][2] + message['username'] +
                        message['message_type'] + message['message'] + mac_field_header + message["mac_field"])

                    logger.info(
                        f"User: {message['username'].decode('utf-8')} send message to user: {clients[partner_socket]['username']} and message is: { message['message'] } message authentication code: {message['mac_field'].decode('utf-8')}")
                    print( f"User: {message['username'].decode('utf-8')} send message to user: {clients[partner_socket]['username']} and message is: { message['message']} message authentication code: {message['mac_field'].decode('utf-8')}")
                    continue
...
if __name__ == '__main__':
    SERVER_RSA_PRIVATE,SERVER_RSA_PUBLIC = get_private_and_public_rsa_keys()
